...
namespace Business.Commands.Classifications
{
    ...
    {
        public AddClassificationCommandValidator(CctDbContext db)
        {
            RuleFor(e => e.Code)
                .NotEmpty()
                .Must(e => !db.Classification.Any(c => c.Code == e)).WithLocalizedStringMessage(typeof(Core.Resources.Validation), "CodeAlreadyExists")
                ;
    ...
    public class AddClassificationCommandHandler : ICommandHandler<AddClassificationCommand>
    {
        ...
        public AddClassificationCommandHandler(CctDbContext db)
        {
            _db = db;
        }
        ...
        {
            await _db.Classification.AddAsync(new Classification()
            {
                Code = command.Code,
